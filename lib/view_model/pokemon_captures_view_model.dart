...
class PokemonCapturesViewModel extends ChangeNotifier {
  ...
  void loadPokemonsCaptures() async {
    try {
      List<String> pokemonsDirs = await FileSystemUtils.getExternalDirectoriesList();
  ...
  void _loadPokemonsCapturesModel(List<String> pokemonsDirs) async {
    for (var pokemonDir in pokemonsDirs) {
      ...
      Pokemon pokemon = await _pokemonProvider.getPokemonById(int.parse(pokemonId));
      List<String> pokemonsCapturePaths = await FileSystemUtils.getExternalFolderList(pokemonFolderName);
